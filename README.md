...
- `sample_contacts.csv` — a tiny example CSV matching the app's expected headers (`name,email,company`). The `company` column is optional.
...
- In the message, you can use placeholders like `[name]` and `[company]` (if provided in CSV) for personalization.
